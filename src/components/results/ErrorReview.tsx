  ...
  return (
    <div className="space-y-6 md:space-y-8">
      <div className="flex items-center gap-3 md:gap-5 mb-6 md:mb-10">
        <div className="p-2 md:p-4 bg-red-100 rounded-xl md:rounded-[2rem] shadow-sm">
          <AlertCircle className="w-5 h-5 md:w-10 md:h-10 text-red-600" />
        </div>
        <div>
          <h2 className="text-xl md:text-4xl font-black tracking-tighter text-slate-900 uppercase leading-none">Análisis de Errores</h2>
          <p className="text-slate-500 font-bold text-[9px] md:text-sm tracking-wide mt-1">Revisión profunda de los ítems con intentos agotados.</p>
        </div>
      </div>
        ...
        {incorrectResponses.map((response, idx) => {
          ...
          return (
            <AccordionItem key={question.id} value={question.id} className="border-none rounded-2xl md:rounded-[2rem] px-4 md:px-8 bg-white shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden academic-shadow">
              <AccordionTrigger className="hover:no-underline py-6 md:py-10 group">
                <div className="flex items-start text-left gap-4 md:gap-6 pr-4 md:pr-8">
                  <span className="shrink-0 w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-red-50 text-red-600 flex items-center justify-center font-black text-base md:text-xl shadow-inner group-hover:bg-red-600 group-hover:text-white transition-colors">
                    {idx + 1}
                  </span>
                  <div className="space-y-1 md:space-y-2">
                    <span className="text-[8px] md:text-[10px] font-black uppercase text-primary/60 tracking-[0.2em]">{question.categoria}</span>
                    <h3 className="font-black text-base md:text-xl leading-snug pr-2 md:pr-6 text-slate-800 tracking-tight">{question.pregunta}</h3>
                  </div>
                </div>
              </AccordionTrigger>
              <AccordionContent className="pb-8 md:pb-12 pt-2 md:pt-4">
